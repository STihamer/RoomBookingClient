...
export class DataService {
  ...
  private bookings: Array<Booking> = Array<Booking>();
  ...
  updateUser(user: User): Observable<User> {
    const originalUser: any = this._users.find(u => u.id === user.id);
  ...
  updateRoom(room: Room): Observable<Room> {
    const originalRoom: any = this._rooms.find(r => r.id === room.id);
  ...
  deleteRoom(id: number): Observable<any> {
    const room: any = this._rooms.find(r => r.id === id);
    this._rooms.splice(this._rooms.indexOf(room), 1);
    return of(null);
  }

  deleteUser(id: number): Observable<any> {
    const user: any = this._users.find(r => r.id === id);
  ...
  saveBooking(booking: Booking): Observable<Booking> {
    const existingBooking: any = this.bookings.find(b => b.id === booking.id);
  ...
  deleteBooking(id: number): Observable<any> {
    const booking: any = this.bookings.find(b => b.id === id);
  ...
  constructor() {
    ...
    this._rooms.push(room1)
    this._rooms.push(room2)
